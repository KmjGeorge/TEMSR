...
def main(args):
    ...
    for i, img_path in enumerate(img_list_gt):
        ...
        if args.channel == 1:
            img_gt = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255.
        else:
        ...
        if args.channel == 1:
            img_restored = cv2.imread(img_path_restored, cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255.
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--channel', type=int, default=3, help='Channel of the image')
