...
class MAEIRModel(BaseModel):
    """Base SR model for single image super-resolution."""

    def __init__(self, opt):    # 初始化，定义网络，读取权重
        super(MAEIRModel, self).__init__(opt)
        # define network
        self.net_g = build_network(opt['network_g'])
        self.net_g = self.model_to_device(self.net_g)
        self.print_network(self.net_g)
        assert self.opt.get('pretrain_mae_ckpt')     # mae encoder
        self.load_network(self.net_g, self.opt.get('pretrain_mae_ckpt'), False, 'model')

        load_path = self.opt['path'].get('pretrain_network_g', None)  # mae decoder
    ...
    def setup_optimizers(self):     # 设置optimizer。不同的params可以设置多组不同的optimizer
        train_opt = self.opt['train']
        optim_params = []
    ...
    def optimize_parameters(self, current_iter):   # 定义了一个完整的train step，包括forward，loss计算，backward，参数优化
        ...
        if isinstance(self.net_g, (torch.nn.parallel.DataParallel, torch.nn.parallel.DistributedDataParallel)):
            self.output = self.net_g.module.unpatchify(self.output)
        else:
            self.output = self.net_g.unpatchify(self.output)
        # if isinstance(self.net_g, (torch.nn.parallel.DataParallel, torch.nn.parallel.DistributedDataParallel)):
        #     self.gt_patchified = self.net_g.module.patchify(self.gt)
        # else:
        #     self.gt_patchified = self.net_g.patchify(self.gt)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if isinstance(self.net_g, (torch.nn.parallel.DataParallel, torch.nn.parallel.DistributedDataParallel)):
                visual_img = self.net_g.module.unpatchify(visuals['result'])
            else:
                visual_img = self.net_g.unpatchify(visuals['result'])
            sr_img = tensor2img([visual_img])
