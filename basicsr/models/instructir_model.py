...
class InstructIRModel(BaseModel):       # 继承于BaseModel，BaseModel提供所有Model共用的一些函数
    ...
    def __init__(self, opt):    # 初始化，定义网络，读取权重
        ...
        # load pretrained models   如果需要，加载预训练权重
    ...
    def init_training_settings(self):   # 初始化训练相关配置，loss，optimizer和scheduler
        self.net_g.train()
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img):   # 验证流程。dist表示多卡一起做验证，而nondist只有单卡
        if self.opt['rank'] == 0:
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if with_metrics:
                ...
                for name, opt_ in self.opt['val']['metrics'].items():
                    self.metric_results[name] += calculate_metric(metric_data, opt_)  # 根据yml中metrics的配置，调用对应的metrics函数，返回值为计算得到的指标
            if use_pbar:
                pbar.update(1)
    ...
    def _log_validation_metric_values(self, current_iter, dataset_name, tb_logger):  # 控制如何打印validation结果
    ...
    def save(self, epoch, current_iter):   # 保存网络(.pth)和训练状态(.state)
        if hasattr(self, 'net_g_ema'):
