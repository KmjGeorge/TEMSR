...
class InstructIR_head(nn.Module):
    """
    ...
    def __init__(self, img_channel=3, width=16, blk_nums=[2, 2, 2, 2], txtdim=256):
        super().__init__()
    ...
    def forward(self, x, txtembd):
        B, C, H, W = x.shape
        res = x
        for encoder, enc_mod in zip(self.encoders, self.enc_cond):
            x = encoder(x)
            x = enc_mod(x, txtembd)
        x = x + res
        x = self.ending(x)
...
if __name__ == '__main__':
    ...
    model = InstructIR_head(img_channel=1, width=64, blk_nums=[4, 4, 4, 4],
                            txtdim=512).cuda()
    summary(model, input_size=[(64, 128, 128), (512,)])  # Text 512: Total params: 9,501,025  32 1118 1 1111
