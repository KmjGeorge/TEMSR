...
class Restormer_Backbone(nn.Module):
    def __init__(self,
        ...
        if output_layer:
            self.recon = nn.Conv2d(out_channels, inp_channels, 3, 1, 1)
        else:
            self.recon = None
    ...
    def forward(self, inp_img):
        ...
        if self.recon:
            return self.recon(out_dec_level1[:, :, :H, :W])
        else:
            return out_dec_level1[:, :, :H, :W]


if __name__ == '__main__':
    net = Restormer_Backbone(inp_channels=1, out_channels=64, dim=48, num_blocks=[4, 6, 6, 8], num_refinement_blocks=4,
                             heads=[1, 2, 4, 8], ffn_expansion_factor=2.66, bias=False, LayerNorm_type='WithBias',
                             dual_pixel_task=False, global_residual=False, output_layer=True)
    weights = torch.load('../../pretrained_models/500000_PretrainEncoder.pth')
    for k in weights.keys():
        print(k)
    msg = net.load_state_dict(weights, strict=False)
    print(msg)
